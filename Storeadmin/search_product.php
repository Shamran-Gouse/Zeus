...
if(isset($_POST['search'])){
	...
	$query = "SELECT * FROM products WHERE id LIKE '%$searchq%' OR product_name LIKE '%$searchq%' OR price LIKE '%$searchq%' OR details LIKE '%$searchq%'";
	$result = mysqli_query($db_conx, $query);

	if(mysqli_num_rows($result) == 0){ // evaluate the count
	
		echo '<script type="text/javascript">
				location.replace("search_product.php?error");
			  </script>';
		 //header("location: search_product.php?error"); 
		 exit();
	}
	else
	{
		while($row = mysqli_fetch_assoc($result)){
			...
			$output .= '<tr>
			<td><a href="edit_Products.php?pid='.$id.'">'.$id.'</a></td>
			<td><a href="edit_Products.php?pid='.$id.'">'.$product_name.'</a></td>
			<td>'.$quantity.'</td>
			<td>'.$price.'</td>
			<td>'.$details.'</td>
			<td>'.$date_added.'</td>
...
<head>
    ...
    <title>Zeus - Product Search</title>
        ...
        <div id="page-wrapper">
            ...
            <div class="container-fluid">
				...
				<ul class="nav nav-tabs">
				  <li class="active"><a href="search_product.php"> Product </a></li>
